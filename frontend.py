...
menu = ["Employee Profile", "Task Tracking", "Goal Setting", "Leaderboard", "Business Insights", "Analytics Dashboard"]
...
if choice == "Analytics Dashboard":
    ...
    st.write("Overview of key performance metrics and trends.")

    # Employees
    employees_data = db.read_employees()
    num_employees = len(employees_data) if employees_data else 0

    # Tasks
    all_tasks = []
    if employees_data:
        for emp in employees_data:
            emp_tasks = db.read_tasks_by_employee(emp[0])
            if emp_tasks:
                all_tasks.extend(emp_tasks)
    num_tasks = len(all_tasks)
    total_hours = sum([task[3] for task in all_tasks]) if all_tasks else 0

    # Goals
    all_goals = []
    if employees_data:
        for emp in employees_data:
            emp_goals = db.read_goals_by_employee(emp[0])
            if emp_goals:
                all_goals.extend(emp_goals)
    num_goals = len(all_goals)

    col1, col2, col3 = st.columns(3)
    col1.metric("Total Employees", num_employees)
    col2.metric("Total Tasks", num_tasks)
    col3.metric("Total Goals", num_goals)

    # Task Status Distribution
    if all_tasks:
        import collections
        status_counts = collections.Counter([task[6] for task in all_tasks])
        st.subheader("Task Status Distribution")
        st.bar_chart(pd.DataFrame.from_dict(status_counts, orient='index', columns=['Count']))

        # Tasks Over Time
        df_tasks = pd.DataFrame(all_tasks, columns=["ID", "Employee ID", "Date", "Duration", "Description", "Priority", "Status", "Outcome"])
        df_tasks['Date'] = pd.to_datetime(df_tasks['Date'])
        tasks_by_date = df_tasks.groupby(df_tasks['Date'].dt.date).size()
        st.subheader("Tasks Logged Over Time")
        st.line_chart(tasks_by_date)

        # Average Task Duration
        avg_duration = df_tasks['Duration'].mean()
        st.metric("Average Task Duration (hrs)", f"{avg_duration:.2f}")

    # Goal Status Distribution
    if all_goals:
        status_counts = collections.Counter([goal[6] for goal in all_goals])
        st.subheader("Goal Status Distribution")
        st.bar_chart(pd.DataFrame.from_dict(status_counts, orient='index', columns=['Count']))

    st.markdown("---")
    st.write("This dashboard provides a summary of employee, task, and goal metrics, along with visualizations for status and trends.")
